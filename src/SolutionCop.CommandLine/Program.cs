    ...
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            if (Parser.Default.ParseArguments(args, commandLineParameters))
            {
                ...
                Console.Out.WriteLine("INFO: Loading config file {0}", pathToConfigFile);
                ...
                if (errors.Any())
                {
                    ...
                    if (commandLineParameters.BuildServerType == BuildServer.TeamCity)
                    {
                        ...
                        Console.WriteLine("##teamcity[testFailed message='FAILED - {0}' details='{1}']", EscapeForTeamCity(Path.GetFileName(pathToConfigFile)), string.Join("|r|n", extendedErrorsInfo.Select(EscapeForTeamCity)));
                        Console.WriteLine("##teamcity[buildStatus text='FAILED - {0}']", EscapeForTeamCity(Path.GetFileName(pathToConfigFile)));
                    }
                }
                else
                {
                    ...
                    {
                        // TODO
                        Console.WriteLine("##teamcity[buildStatus status='SUCCESS' text='PASSED - {0}']", EscapeForTeamCity(Path.GetFileName(pathToConfigFile)));
                    }
                }
